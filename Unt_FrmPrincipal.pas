...
uses
  ...
  Vcl.Menus, Vcl.Imaging.jpeg, Vcl.Imaging.pngimage, MidasLib;
...
type
  TFrmPrincipal = class(TForm)
    ...
    procedure pnlPDVClick(Sender: TObject);
  private
...
procedure TFrmPrincipal.pnlPDVClick(Sender: TObject);
var _TelaPDV : TFrmTelaPdv;
begin
  _TelaPDV := TFrmTelaPdv.New;
  try
    _TelaPDV.ShowModal;
  finally
    FreeAndNil(_TelaPdv);
...
procedure TFrmPrincipal.SubGrupos1Click(Sender: TObject);
var _RelSubGrupo : TFrmRelSubGrupo;
begin
  _RelSubGrupo := TFrmRelSubGrupo.New;
  try
    _RelSubGrupo.ShowModal;
  finally
    FreeAndNil(_RelSubGrupo);
  end;
end;
